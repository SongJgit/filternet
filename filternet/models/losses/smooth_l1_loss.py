...
@weighted_loss
def focal_smooth_l1_loss(pred: Tensor,
                         target: Tensor,
                         beta: float = 1.0,
                         gamma: float = 1.0,
                         alpha: float = 0.2,
                         activate: str = 'sigmoid') -> Tensor:
    assert beta > 0
    if target.numel() == 0:
        return pred.sum() * 0
    ...
    loss = torch.where(diff < beta, 0.5 * diff * diff / beta, diff - 0.5 * beta)
    if activate == 'sigmoid':
        scale_factor = (torch.sigmoid(alpha * loss)) ** gamma  # [0, 1]
    elif activate == 'tanh':
        scale_factor = (torch.tanh(alpha * loss)) ** gamma  # [0, 1]
...
class FocalSmoothL1Loss(nn.Module):
    ...
    def forward(self,
        ...
        assert reduction_override in (None, 'none', 'mean', 'sum')
        reduction = (reduction_override if reduction_override else self.reduction)
        loss_bbox = self.loss_weight * focal_smooth_l1_loss(pred,
                                                            target,
                                                            weight,
                                                            beta=self.beta,
                                                            alpha=self.alpha,
