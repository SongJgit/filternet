...
@weighted_loss
def eiou_loss(pred: Tensor, target: Tensor, smooth_point: float = 0.1, eps: float = 1e-7) -> Tensor:
    ...
    # Intersection
    intersection = (ix2 - ex1) * (iy2 - ey1) + (xmin - ex1) * (ymin - ey1) - (ix1 - ex1) * (ymax - ey1) - (
        xmax - ex1) * (iy1 - ey1)
    # Union
...
class BoundedIoULoss(nn.Module):
    ...
    def __init__(self, beta: float = 0.2, eps: float = 1e-3, reduction: str = 'mean', loss_weight: float = 1.0) -> None:
        super().__init__()
        self.beta = beta
        self.eps = eps
        self.reduction = reduction
...
class EIoULoss(nn.Module):
    ...
    def forward(self,
        ...
        if weight is not None and weight.dim() > 1:
            assert weight.shape == pred.shape
            weight = weight.mean(-1)
        loss = self.loss_weight * eiou_loss(pred,
                                            target,
                                            weight,
...
class SIoULoss(nn.Module):
    ...
    def forward(self,
        ...
        if weight is not None and weight.dim() > 1:
            ...
            assert weight.shape == pred.shape
            weight = weight.mean(-1)
        loss = self.loss_weight * siou_loss(pred,
                                            target,
                                            weight,
                                            eps=self.eps,
                                            reduction=reduction,
