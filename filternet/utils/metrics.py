...
def _mean_squared_error_update(preds: Tensor, target: Tensor, dim: int | Tuple | None = None) -> Tuple[Tensor, int]:
...
class MSE(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor) -> None:
        ...
        sum_squared_error, n_obs = _mean_squared_error_update(preds, target, self.dim)
...
class AxisMSE(Metric):
    ...
    def __init__(self, num_axis: int, dim: int | Tuple | None = None, squared: bool = True):
        """Calculate the MSE for each step along multiple axis. like x axis
        MSE.
        ...
        for i in range(num_axis):
            self.add_state(f'sum_squared_error_{i}', default=torch.tensor(0.0), dist_reduce_fx='sum')
        self.add_state('total', default=torch.tensor(0.0), dist_reduce_fx='sum')
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor) -> None:
        ...
        assert preds.shape == target.shape
        assert preds.shape[1] == self.num_axis, 'Compute axis must be equal to shape[1]'
        for i in range(self.num_axis):
            sum_squared_error, n_obs = _mean_squared_error_update(preds[:, i, ...], target[:, i, ...], dim=self.dim)
    ...
    def compute(self) -> List[torch.Tensor]:
        ...
        for i in range(self.num_axis):
            ...
            res.append(_mean_squared_error_compute(sum_squared_error, self.total, self.squared))
        return res
...
#     """generate metric for prediction.
...
def compute_metric(preds: torch.Tensor, targets: torch.Tensor) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """_summary_
...
def print_metrics(predict: Tensor, target: Tensor, name: str = None):
    ...
    print(f"{name} - MSE : {scalar['mse_dB']}[dB]")
    print(f"{name} - MSE : {scalar['mse']}")
    print(f"{name} - RMSE : {scalar['rmse']}")
    return scalar, curves
