...
def _format_lat_lon(lat: list, lon: list):
    ...
    lat_lon = ['            {},{},1'.format(lo, la) for la, lo in zip(lat, lon)]
    return '\n'.join(lat_lon)
...
def read_gps(data_dir, data_date, use_rtk=False):
    ...
    filepath = osp.join(data_dir, 'sensor_data', data_date, filename)
    print(f'{filepath}')
...
def read_imu(data_dir, data_date):
    ...
    filepath = osp.join(data_dir, 'sensor_data', data_date, 'ms25.csv')
...
def read_ground_truth(dir, dataset_date, truncation=-1):
    ...
    filepath_cov = osp.join(dir, 'sensor_data', dataset_date, 'odometry_cov_100hz.csv')

    # data_date = osp.split(osp.dirname(data_dir))[-1]
    # filepath_gt = f"dir/ground_truth/groundtruth_{dataset_date}.csv"
    filepath_gt = osp.join(dir, 'ground_truth', f'groundtruth_{dataset_date}.csv')
...
def read_euler(data_dir, data_date):
    filepath = osp.join(data_dir, 'sensor_data', data_date, 'ms25_euler.csv')
...
def read_wheels(data_dir, data_date):
    ...
    filepath = osp.join(data_dir, 'sensor_data', data_date, 'wheels.csv')
...
def preprocess(dir, data_date, use_rtk=False):
    ...
    for k in range(1, len(t)):
        ...
        wheel.append(torch.tensor([v_left_wheel[wheel_counter], v_right_wheel[wheel_counter], theta, omega_imu]))
        ...
        if gps_counter != prev_gps_counter:
            filtered_gps.append(torch.tensor([gps_x[gps_counter], gps_y[gps_counter]]))
            prev_gps_counter = gps_counter
        else:
            filtered_gps.append(torch.tensor([torch.nan, torch.nan]))

        if wheel_counter != prev_wheel_counter:
            filtered_wheel.append(torch.tensor([v_left_wheel[wheel_counter], v_right_wheel[wheel_counter]]))
            prev_wheel_counter = wheel_counter
        else:
            filtered_wheel.append(torch.tensor([torch.nan, torch.nan]))
        gt.append(torch.tensor([x_true[ground_truth_counter], y_true[ground_truth_counter]]))
...
if __name__ == '__main__':
    ...
    opt.add_argument('--data_dir', default='./data/NCLT', help='path to data')
    opt.add_argument('--tgt_dir', default='./data/nclt_fusion', help='save path to data')
    ...
    listdir = os.listdir(osp.join(args.data_dir, 'sensor_data'))
    processed = []
    ...
    for date in tqdm(listdir):
        print(date)
        processed.append(preprocess(args.data_dir, date, args.use_rtk))
    t = range(processed.__len__())
    if args.random_split:

        generator = torch.Generator().manual_seed(args.random_seed)
        train_dataset, test_dataset, val_dataset = random_split(processed, [0.8, 0.1, 0.1])
    else:
        # For easy comparison with mte-546-project, specify that the
        # test contains '2013-04-05' and the rest are random cuts.
        train = [
            '2012-08-20', '2012-02-19', '2012-08-04', '2012-03-25', '2012-06-15', '2012-01-22', '2012-01-15',
            '2012-01-08', '2012-02-04', '2012-02-02', '2012-05-11', '2012-04-29', '2012-02-18', '2012-03-31',
            '2012-12-01', '2012-09-28', '2012-02-05', '2012-11-17', '2012-02-12', '2012-05-26', '2012-03-17',
            '2012-10-28']
    ...
    torch.save(list(train_dataset), f'./data/nclt_fusion/train{filename}.pt')
    torch.save(list(test_dataset), f'./data/nclt_fusion/test{filename}.pt')
    torch.save(list(val_dataset), f'./data/nclt_fusion/val{filename}.pt')
