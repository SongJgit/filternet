...
def main(args):
    ...
    for date in dates:
        ...
        if args.lb3 or args.all:
            ...
            cmd = ['wget', '--continue', '%s/images/%s_lb3.tar.gz' % (base_dir, date), '-P', 'images']
        ...
        if args.sen or args.all:
            ...
            cmd = [
                'wget', '--continue',
                '%s/sensor_data/%s_sen.tar.gz' % (base_dir, date), '-P', './data/NCLT/sensor_data_tar']
            print('Calling: ', ' '.join(cmd))
            subprocess.call(cmd)
        if args.vel or args.all:
            ...
            cmd = [
                'wget', '--continue',
                '%s/velodyne_data/%s_vel.tar.gz' % (base_dir, date), '-P', './data/NCLT/velodyne_data']
            print('Calling: ', ' '.join(cmd))
            subprocess.call(cmd)
        if args.hokuyo or args.all:
            ...
            cmd = [
                'wget', '--continue',
                '%s/hokuyo_data/%s_hokuyo.tar.gz' % (base_dir, date), '-P', './data/NCLT/hokuyo_data']
            print('Calling: ', ' '.join(cmd))
            subprocess.call(cmd)
        if args.gt or args.all:
            ...
            cmd = [
                'wget', '--continue',
                '%s/ground_truth/groundtruth_%s.csv' % (base_dir, date), '-P', './data/NCLT/ground_truth']
            print('Calling: ', ' '.join(cmd))
            subprocess.call(cmd)
        if args.gt_cov or args.all:
            ...
            cmd = [
                'wget', '--continue',
                '%s/covariance/cov_%s.csv' % (base_dir, date), '-P', './data/NCLT/ground_truth_cov']
            print('Calling: ', ' '.join(cmd))
            subprocess.call(cmd)
